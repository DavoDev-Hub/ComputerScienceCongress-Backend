...
export const registrarAsistencia = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json({
      ...
      id_alumno: nuevaAsistencia.id_alumno,
      id_actividad: nuevaAsistencia.id_actividad ?? null,
      id_conferencia: nuevaAsistencia.id_conferencia ?? null,
      fecha_asistencia: nuevaAsistencia.fecha_asistencia,
    });
  } catch (error) {
  ...
  try {
    ...
    const resultado = alumnos.map((alumno) => {
      ...
      return {
        ...
        totalAsistencias:
          detalleConferencias.length + detalleActividades.length,
        detalle: {
...
export const getAsistenciasPorAlumno = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al obtener asistencias del alumno" });
  }
};
...
export const getRecentAttendances = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al obtener asistencias recientes" });
  }
};
...
export const deleteAsistencia = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ mensaje: "Asistencia eliminada correctamente" });
  } catch (error) {
